# VERSION A9b – CON TRES OPCIONES ESTA ES LA FINAL ULTIMA QUE FUNCIONA
# 1: Ad-hoc extension
# 2: Ad-hoc + bootstrap uncertainty bands
# 3: Solo bootstrap uncertainty bands
...
# ──────────────────────────────────────────────────────────────────────────────
# METHOD CHOICE: ahora hay 3 opciones
# ──────────────────────────────────────────────────────────────────────────────
print("\n" + "═"*80)
print("SELECCIÓN DE MÉTODO PARA MANEJAR LAS COLAS / INCERTIDUMBRE")
print("═"*80)
print("1 → Extensión ad-hoc de las marginales (simple y rápido)")
print("2 → Extensión ad-hoc + bandas de incertidumbre bootstrap")
print("3 → Solo bandas de incertidumbre bootstrap (más robusto)")
print("")

while True:
    try:
        entrada = input("Elige el método (1, 2 o 3) [por defecto 1]: ").strip()
        if entrada == "":
            method_choice = 1
            print("→ Método por defecto: 1 (extensión ad-hoc)")
            break
        method_choice = int(entrada)
        if method_choice in [1, 2, 3]:
            print(f"→ Método seleccionado: {method_choice}")
            break
        else:
            print("Por favor, introduce 1, 2 o 3.")
    except ValueError:
        print("Entrada no válida. Introduce 1, 2 o 3 (o Enter para defecto).")

print("")

# Parámetros configurables
margin_factor = 0.05       # Para métodos 1 y 2: 5% extra por cola
n_boot = 100               # Para métodos 2 y 3: número de re-muestras bootstrap
...
# ──────────────────────────────────────────────────────────────────────────────
# Unified plotting function – actualizada para 3 métodos
# ──────────────────────────────────────────────────────────────────────────────
...
def generate_four_plots(case_name, N_data, Delta_data, color_data='darkblue', color_sim='darkorange',
    ...
    # Forzamos método 3 en el caso Beta si alguien eligió 1 o 2
    if "Beta" in case_name and current_method in [1, 2]:
        current_method = 3
        print(f"   Nota: Para el caso Beta → forzado método 3 (bootstrap)")
    ...
    if use_adhoc:
        # Extensión ad-hoc (igual que método 1 original)
    ...
    if use_bootstrap:
        # Bootstrap para incertidumbre en θ
        thetas_boot = []
        ...
        # Si también usamos ad-hoc → la central es la ad-hoc, sino usamos la media bootstrap
        if not use_adhoc:
            theta_central = theta_mean

    method_str = {1: "Ad-hoc only", 2: "Ad-hoc + Bootstrap bands", 3: "Bootstrap only"}[current_method]

    # ── Figura 1: Datos + curvas percentílicas ──────────────────────────────
    plt.figure(figsize=(10, 6))
    plt.scatter(x_data, Delta_data, color=color_data, alpha=0.65, s=36, label="Original data")

    if Q_N is not None and cdf_Delta is not None:
        # Siempre dibujamos la curva central
        ...
        # Bandas solo si usamos bootstrap
    ...
    plt.title(f"{case_name} – Data & percentile curves\n({method_str})")
    plt.grid(True, alpha=0.3)
    plt.legend(ncol=3, fontsize=8)
    plt.xlim(x_min, x_max)
    plt.ylim(delta_min_plot, delta_max_plot)
    plt.tight_layout()
    plt.savefig(f'/content/FIGURES/fig_{fig_counter:02d}_{fig_prefix}_data_percentiles.png', dpi=300, bbox_inches='tight')
    fig_counter += 1
    plt.show()

    # ── Figura 2: Empirical copula ── (sin cambios)
    ...
    # ── Figura 3: Frank copula fit ── (actualizada)
    plt.figure(figsize=(8, 8))
    plt.scatter(u, v, s=45, alpha=0.75, color=color_data, edgecolor='none', label='Data')

    # Curva central
    v_curve = conditional_quantile_frank(percentiles[0], u_grid, theta_central)  # solo para tener la escala
    for p in percentiles:
    ...
    # ── Figura 4: Simulated vs original + curvas ── (similar a fig1)
    if Q_N is not None and cdf_Delta is not None and Q_Delta is not None:
        ...
        plt.xlabel(xlabel_phys)
        plt.ylabel(r"$\Delta\sigma$")
        plt.title(f"{case_name} – Original vs Frank simulation + curves\n({method_str})")
        plt.grid(True, alpha=0.3)
        plt.legend(ncol=3, fontsize=8)
        plt.xlim(x_min, x_max)
        plt.ylim(delta_min_plot, delta_max_plot)
...
# ──────────────────────────────────────────────────────────────────────────────
# Global settings & casos (el resto del código sigue igual)
# ──────────────────────────────────────────────────────────────────────────────
fig_counter = 1
...
# ──────────────────────────────────────────────────────────────────────────────
# CASE 2 – Beta simulated model (CORREGIDO: distribuciones paramétricas)
# ──────────────────────────────────────────────────────────────────────────────
print("\n" + "═"*80)
...
np.random.seed(123)
n_beta = 1000  # Use same n as other cases
...
# Para N_beta: distribución uniforme extendida
N_min, N_max = 0.0, 1.0
margin_N = 0.01  # 1% margin
Q_N_beta = lambda u: np.clip(uniform.ppf(u, loc=N_min, scale=N_max-N_min),
                             N_min - margin_N,
                             N_max + margin_N)

# Para Delta_beta: ajuste paramétrico Beta con márgenes
# Estimar parámetros alpha, beta por momentos
mean_delta = np.mean(Delta_beta)
var_delta = np.var(Delta_beta)

# Fórmula de momentos para distribución Beta
if var_delta > 0 and 0 < mean_delta < 1:
    ...
    # Valores por defecto si no se pueden estimar
    alpha_beta, beta_beta = 2.0, 2.0

print(f"Parámetros Beta estimados: alpha={alpha_beta:.2f}, beta={beta_beta:.2f}")

# Crear funciones cuantílicas con márgenes
delta_beta_min = 0.0
delta_beta_max = 1.0
margin_delta = 0.05  # 5% margin
