# -*- coding: utf-8 -*-
"""modeloWeibullBOOK.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/12EC0Hy5_SHERtQEZdwMCNrYeC2nZonGs
"""

# Modelo Mixto USADO EN EL LIBRO
# FUNCIONA
...
if generate_synthetic == 'y':
    ...
    fig, axes = plt.subplots(1, n_plots + 1, figsize=(6 * (n_plots + 1), 6), sharex=True, sharey=True)

    # Recopilar todos los datos relevantes para calcular límites robustos
    all_N = [cycles_data]
    all_stress = [stress_data]
    for i in range(n_plots):
        df = all_synthetic_data[i]
        all_N.append(df['N'].values)
        all_stress.append(df['Deltasigma'].values)
    all_N = np.concatenate(all_N)
    all_stress = np.concatenate(all_stress)

    # Filtrar valores válidos (positivos y finitos)
    valid_mask = (all_N > 0) & np.isfinite(all_N) & (all_stress > 0) & np.isfinite(all_stress)
    all_N_valid = all_N[valid_mask]
    all_stress_valid = all_stress[valid_mask]

    if len(all_N_valid) == 0:
        print("⚠ No hay valores válidos para plotear")
        all_N_valid = np.array([1.0])
        all_stress_valid = np.array([0.5])

    # Límites ampliados basados en TODOS los datos válidos
    x_min_global = all_N_valid.min() * 0.05   # Más margen inferior
    x_max_global = all_N_valid.max() * 50.0   # Mucho más margen superior para valores sintéticos extremos
    y_min_global = max(all_stress_valid.min() * 0.8, 0.01)
    y_max_global = min(all_stress_valid.max() * 1.2, 1.5)

    # Plot observed data (primer subplot)
    ax = axes[0]
    valid_obs = (cycles_data > 0) & np.isfinite(cycles_data)
    ax.scatter(cycles_data[valid_obs], stress_data[valid_obs],
               c='black', s=80, alpha=0.9,
               label='Observed data', zorder=20, marker='o',
               edgecolors='white', linewidths=1.5)
    ax.set_xscale('log')
    ax.set_xlim(x_min_global, x_max_global)
    ax.set_ylim(y_min_global, y_max_global)
    ax.set_xlabel('Cycles to Failure (N)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Stress (Δσ, dimensionless)', fontsize=12, fontweight='bold')
    ax.set_title(f'Observed Data\n({np.sum(valid_obs)} valid obs)', fontsize=13, fontweight='bold')
    ax.grid(True, alpha=0.3, which='both')
    ...
    for i in range(n_plots):
        ...
        df = all_synthetic_data[i]
        N_synth = df['N'].values
        stress_synth = df['Deltasigma'].values
        valid_synth = (N_synth > 0) & np.isfinite(N_synth) & (stress_synth > 0) & np.isfinite(stress_synth)
        n_valid = np.sum(valid_synth)
        print(f"  Synthetic {i+1}: {n_valid}/{len(N_synth)} puntos válidos")

        if n_valid > 0:
            ax.scatter(N_synth[valid_synth], stress_synth[valid_synth],
                       c=colors[i], s=70, alpha=0.85,
                       label=f'Synthetic {i+1} ({n_valid} pts)', zorder=20,
                       marker=markers[i], edgecolors='black', linewidths=1.2)
        else:
            ax.text(0.5, 0.5, 'No valid points', transform=ax.transAxes,
        ...
        if i == 0:
            ax.set_ylabel('Stress (Δσ, dimensionless)', fontsize=12, fontweight='bold')
        ax.set_title(f'Synthetic Dataset {i+1}\n({n_valid} valid obs)', fontsize=13, fontweight='bold')
        ax.grid(True, alpha=0.3, which='both')
        ax.legend(fontsize=10)

    plt.suptitle('Comparison: Observed Data vs First 3 Synthetic Datasets '
                 f'(Total valid points across all: {len(all_N_valid)})',
                 fontsize=16, fontweight='bold', y=0.98)
    plt.tight_layout()
    comparison_plot = f'{synthetic_dir}/synthetic_vs_observed_comparison.png'
    plt.savefig(comparison_plot, dpi=250, bbox_inches='tight')
    plt.show()
