...
Original file is located at
    https://colab.research.google.com/drive/1bdgrpwPQHGNTUdCXNnWPdGi7gGmvNuPF
"""

# FatigueSNBayesPython
# -*- coding: utf-8 -*-
...
# - Lower bounded problem (N > 0) → Weibull distribution for minima
# - Castillo-Canteli Weibull 3-parameter model as described in .tex
# ============================================================================
...
print("Castillo-Canteli 3-parameter Weibull Formulation")
print("="*70)
...
if use_example:
    ...
    cycles_data = np.array([
        37,72,74,76,83,85,105,109,120,123,143,203,206,217,257,201,216,226,252,
        257,295,311,342,356,451,457,509,540,680,1129,1246,1258,1460,1492,2400,
        2410,2590,2903,3330,3590,3847,4110,4820,5560,5598,6710,9930,12600,
        15580,16190,17280,18620,20300,24900,26260,27940,36350,48420,50090,
        67340,102950,280320,339830,366900,485620,658960,896330,1241760,1250200,
        1329780,1399830,1459140,3294820,12709000,14373000
    ])
    ...
    if IN_COLAB:
        ...
        if len(uploaded) == 0:
            ...
            if not os.path.exists(filename):
                ...
                print(f"  ✓ File verified at: {os.path.abspath(filename)}")
    else:
    ...
    if not use_example:
        ...
        try:
            ...
            if filename.endswith(('.txt', '.dat')):
                ...
                print(f"  ✓ Parsed N: {len(cycles_data)} values")
                print(f"  ✓ Parsed Deltasigma: {len(stress_data)} values")
                print(f"  ✓ Created numpy arrays successfully")
                ...
                for cv_param in cv_params:
                    ...
                    if cv_match:
                        ...
                        print(f"  ✓ Found {cv_param}={config[cv_param]}")

                # target_accept
                accept_match = re.search(r'target_accept\s*=\s*([\d.]+)', content)
                if accept_match:
                    config['target_accept'] = float(accept_match.group(1))
                    print(f"  ✓ Found target_accept={config['target_accept']}")

                dataset_name = filename.split('.')[0]

                print(f"\n  ✓✓✓ R FORMAT FILE LOADED SUCCESSFULLY")
            ...
            if filename.endswith(('.csv', '.xlsx', '.xls')):
                ...
                print(f"\n✓ Data loaded: {len(cycles_data)} observations")
...
if use_example or 'stress_data' not in locals() or 'cycles_data' not in locals():
    if not use_example:
        print("\n⚠ Data not loaded properly, using Holmen example instead.")
...
print(f"  CV(N₀) = {config['cv_N0']:.2f}")
print(f"  CV(Δσ₀) = {config['cv_Delta0']:.2f}")
print(f"  CV(β) = {config['cv_beta']:.2f}")
print(f"  CV(λ) = {config['cv_lambda']:.2f}")
print(f"  CV(δ) = {config['cv_delta']:.2f}")
...
# ============================================================================
# PHASE 1: MAXIMUM LIKELIHOOD ESTIMATION (WEIBULL) - CORREGIDO
# ============================================================================

print("\n" + "="*70)
print("PHASE 1: MAXIMUM LIKELIHOOD ESTIMATION")
print("Castillo-Canteli 3-parameter Weibull Model (from .tex)")
print("="*70)

def calculate_v(N, Delta_sigma, N0, Delta0):
    """
    Calculate dimensionless variable V as per Castillo-Canteli model.
    V = log(N/N0) * (Δσ/Δσ₀)
    """
    return np.log(N / N0) * (Delta_sigma / Delta0)

def weibull_logpdf(v, lambda_param, delta, beta):
    """
    Weibull log-PDF for the 3-parameter Weibull distribution.
    As defined in the .tex document.
    ...
    log_pdf : array-like
        Log-PDF values
    """
    # Only defined for v >= lambda_param
    mask = v >= lambda_param

    # Initialize with -inf for v < lambda_param
    log_pdf = np.full_like(v, -np.inf)

    # For v >= lambda_param, compute Weibull log-PDF
    if np.any(mask):
        v_masked = v[mask]
        z = (v_masked - lambda_param) / delta
        log_pdf[mask] = (np.log(beta) - np.log(delta) +
                         (beta - 1) * np.log(z) -
                         z**beta)
...
def weibull_log_likelihood(params, stress, cycles):
    ...
    Castillo-Canteli dimensionless formulation as in .tex.
    """
    N0, Delta0, lambda_param, delta, beta = params

    # Validations (all must be positive except lambda)
    if N0 <= 0 or Delta0 <= 0 or delta <= 0 or beta <= 0:
        return -np.inf

    # Calculate dimensionless variable V
    V = calculate_v(cycles, stress, N0, Delta0)

    # Check constraint: V >= lambda for all observations
    if np.any(V < lambda_param):
        return -np.inf

    # Compute log-likelihood
    log_lik = weibull_logpdf(V, lambda_param, delta, beta)
...
initial_params = np.array([N0_init, Delta0_init, lambda_init, delta_init, beta_init])

print("\nInitial guess (Castillo-Canteli formulation):")
print(f"  N₀ = {N0_init:.4f}")
print(f"  Δσ₀ = {Delta0_init:.4f}")
print(f"  λ = {lambda_init:.4f}")
print(f"  δ = {delta_init:.4f}")
...
bounds = [
    ...
    (-15.0, 0.0),                         # lambda (typically negative)
    (0.1, 10.0),                          # delta
    (0.5, 15.0)                           # beta
]
...
print("\n" + "="*70)
print("MLE ESTIMATES (CASTILLO-CANTELI WEIBULL MODEL)")
print("="*70)
print(f"  N₀ (reference cycles) = {mle_params[0]:.6f}")
print(f"  Δσ₀ (reference stress) = {mle_params[1]:.6f}")
print(f"  λ (location parameter) = {mle_params[2]:.6f}")
print(f"  δ (scale parameter)    = {mle_params[3]:.6f}")
print(f"  β (shape parameter)    = {mle_params[4]:.6f}")
print(f"\n  Log-likelihood = {-result_global.fun:.2f}")

# Verify constraint satisfaction
V_mle = calculate_v(cycles_data, stress_data, mle_params[0], mle_params[1])
constraint_satisfied = np.all(V_mle >= mle_params[2])
print(f"  Constraint V >= λ satisfied: {'✓' if constraint_satisfied else '✗'}")
if not constraint_satisfied:
    print(f"    Min(V) = {V_mle.min():.6f}, λ = {mle_params[2]:.6f}")
    print(f"    Observations with V < λ: {np.sum(V_mle < mle_params[2])}")

# Save MLE results
mle_estimates_file = 'mle_estimates.txt'
with open(mle_estimates_file, 'w') as f:
    f.write(f"MLE ESTIMATES - CASTILLO-CANTELI WEIBULL MODEL - {dataset_name}\n")
    f.write("="*60 + "\n")
    f.write(f"N0 = {mle_params[0]:.8f}\n")
    f.write(f"Delta0 = {mle_params[1]:.8f}\n")
    f.write(f"lambda = {mle_params[2]:.8f}\n")
    f.write(f"delta = {mle_params[3]:.8f}\n")
    f.write(f"beta = {mle_params[4]:.8f}\n")
    f.write(f"Log-likelihood = {-result_global.fun:.8f}\n")
    f.write(f"Constraint V >= λ satisfied: {constraint_satisfied}\n")

print("\n✓ MLE estimates saved to 'mle_estimates.txt'")
...
N0_mle, Delta0_mle, lambda_mle, delta_mle, beta_mle = mle_params

for perc, color in zip(percentiles_mle, colors_mle):
    N_perc = []
    for stress in stress_range_plot:
        if stress > 0 and Delta0_mle > 0:
            # Calculate v_p for this percentile
            v_p = lambda_mle + delta_mle * ((-np.log(1 - perc)) ** (1/beta_mle))

            # Calculate N from v_p = log(N/N0) * (stress/Delta0)
            # => N = N0 * exp(v_p * Delta0 / stress)
            if v_p > 0:  # Ensure positive
                N_p = N0_mle * np.exp(v_p * Delta0_mle / stress)
            else:
                N_p = np.nan
...
ax.set_xscale('log')
ax.set_title(f'MLE Fit - Castillo-Canteli Weibull Model ({dataset_name})',
...
print("✓ MLE visualization completed!")
...
lambda_mle_val = mle_params[2]
delta_mle_val = mle_params[3]
beta_mle_val = mle_params[4]
...
print(f"  N₀     ~ Normal({N0_mle_val:.4f}, {N0_std:.4f})  [CV={config['cv_N0']:.2f}]")
print(f"  Δσ₀    ~ Normal({Delta0_mle_val:.6f}, {Delta0_std:.6f})  [CV={config['cv_Delta0']:.2f}]")
print(f"  λ      ~ Normal({lambda_mle_val:.4f}, {lambda_std:.4f})  [CV={config['cv_lambda']:.2f}]")
print(f"  δ      ~ Normal({delta_mle_val:.4f}, {delta_std:.4f})  [CV={config['cv_delta']:.2f}]")
print(f"  β      ~ Normal({beta_mle_val:.4f}, {beta_std:.4f})  [CV={config['cv_beta']:.2f}]")

print("\nThese priors will help achieve faster convergence in Bayesian inference.")

# ============================================================================
# PHASE 3: BAYESIAN INFERENCE WITH WEIBULL MODEL - AJUSTADO
# ============================================================================

print("\n" + "="*70)
print("PHASE 3: BAYESIAN INFERENCE")
print("Castillo-Canteli Weibull Model")
print("="*70)
...
with pm.Model() as fatigue_model:
    ...
    # Calculate dimensionless variable V
    V = pt.log(cycles_data / N0) * (stress_data / Delta0)

    # Weibull log-PDF for the 3-parameter Weibull distribution
    # Using the same formulation as in MLE
    # z = (V - lambda_param) / delta
    # log_pdf = log(beta) - log(delta) + (beta-1)*log(z) - z**beta

    # Add small constant to avoid numerical issues
    z = (V - lambda_param + 1e-8) / (delta + 1e-8)

    # Log-likelihood (Weibull 3-parameter as in .tex)
    log_lik = (pt.log(beta) - pt.log(delta) +
               (beta - 1) * pt.log(z + 1e-8) -
               pt.pow(z, beta))

    # Constraint: V >= lambda_param
    constraint = pt.all(V >= lambda_param)

    # Apply constraint using potential
    likelihood = pm.Potential('likelihood',
                              pt.switch(constraint, pt.sum(log_lik), -np.inf))

print("✓ Bayesian model defined with informative priors")
print("  Using Castillo-Canteli 3-parameter Weibull formulation")
...
print("✓ Prior predictive samples generated")
...
var_names = ['N0', 'Delta0', 'lambda_param', 'delta', 'beta']
var_labels = ['N₀ (reference cycles)', 'Δσ₀ (reference stress)', 'λ', 'δ', 'β']
...
print("✓ Prior distributions visualized")
...
print("\nSampling strategy:")
print(f"  • Using informative priors from MLE")
print(f"  • Tune: {config['n_tune']}, Draws: {config['n_draws']}, Chains: {config['n_chains']}")
print(f"  • Target acceptance = {config['target_accept']}")
...
all_rhat_good = True
for var in ['N0', 'Delta0', 'lambda_param', 'delta', 'beta']:
...
if all_rhat_good and all_ess_good and n_divergences == 0:
    print("✓✓✓ EXCELLENT: Model has converged successfully!")
elif all_rhat_good and n_divergences < 50:
    print("✓✓ GOOD: Model convergence is acceptable")
else:
...
# ============================================================================
# PHASE 7: PERCENTILES OF PERCENTILES - AJUSTADO PARA EL NUEVO MODELO
# ============================================================================

print("\n" + "="*70)
print("COMPUTING PERCENTILES OF PERCENTILES")
print("Castillo-Canteli Weibull Formulation")
print("="*70)
...
def compute_percentile(stress, N0, Delta0, lambda_p, delta, beta, prob):
    """
    Compute N_p for given stress and failure probability.
    Using Castillo-Canteli Weibull formulation as in .tex.
    """
    if stress <= 0 or N0 <= 0 or Delta0 <= 0 or delta <= 0 or beta <= 0:
        return np.nan
    if prob <= 0 or prob >= 1:
        return np.nan
    ...
    try:
        # Calculate v_p for this percentile
        v_p = lambda_p + delta * ((-np.log(1 - prob)) ** (1/beta))

        # Calculate N from v_p = log(N/N0) * (stress/Delta0)
        # => N = N0 * exp(v_p * Delta0 / stress)
        if v_p > 0:  # Ensure positive
            N_p = N0 * np.exp(v_p * Delta0 / stress)
        else:
            return np.nan
...
for perc_base_idx, perc_base in enumerate(percentiles_base):
    ...
    for i, stress in enumerate(stress_range):
        ...
        for j, idx in enumerate(sample_indices):
            ...
            N_p = compute_percentile(stress, N0_s, Delta0_s, lambda_s, delta_s, beta_s, perc_base)
...
print("\n✓ Percentiles of percentiles computed!")
...
ax.set_ylabel('Stress (Deltasigma, dimensionless)', fontsize=15, fontweight='bold')
ax.set_xscale('log')
ax.set_title(f'All {total_curves} Percentile Curves ({len(percentiles_base)} Base × {len(percentiles_sub)} Sub) - {dataset_name}\n' +
...
print("✓ Alternative plot completed!")
...
print("\n1. MLE ESTIMATES (Castillo-Canteli Weibull):")
print(f"   N₀ = {mle_params[0]:.6f}")
print(f"   Δσ₀ = {mle_params[1]:.6f}")
print(f"   λ = {mle_params[2]:.3f}")
print(f"   δ = {mle_params[3]:.3f}")
print(f"   β = {mle_params[4]:.3f}")

print("\n2. POSTERIOR MEDIANS:")
print(f"   N₀ = {posterior['N0'].median().values:.6f}")
print(f"   Δσ₀ = {posterior['Delta0'].median().values:.6f}")
print(f"   λ = {posterior['lambda_param'].median().values:.3f}")
print(f"   δ = {posterior['delta'].median().values:.3f}")
print(f"   β = {posterior['beta'].median().values:.3f}")

print("\n3. CONVERGENCE:")
print(f"   R-hat all < 1.01: {'✓' if all_rhat_good else '✗'}")
print(f"   ESS all > 1000: {'✓' if all_ess_good else '✗'}")
print(f"   Divergences: {n_divergences}")
if n_divergences > 0:
    print(f"   Divergence rate: {n_divergences/total_samples*100:.2f}%")

print("\n4. PERCENTILES OF PERCENTILES:")
print(f"   Total curves generated: {len(percentiles_base) * len(percentiles_sub)} ({len(percentiles_base)} base × {len(percentiles_sub)} sub)")
print(f"   Two visualization approaches:")
print(f"     • Main plot: Shaded bands with key curves")
print(f"     • Alternative: All curves individually labeled")

print("\n5. FILES CREATED:")
print("   • data_exploration_weibull.png")
print("   • mle_results.png")
print("   • mle_estimates.txt")
print("   • prior_distributions.png")
print("   • trace_plots.png")
print("   • posterior_distributions.png")
print("   • percentiles_of_percentiles_shaded.png")
print("   • percentiles_of_percentiles_all_curves.png")
...
print("\n✓ Results saved:")
print("  • fatigue_posterior_weibull.nc")
print("  • fatigue_summary_weibull.csv")
print("  • percentiles_of_percentiles.npz")

# ============================================================================
# PHASE 11: SYNTHETIC DATA GENERATION FROM POSTERIOR SAMPLES - CORREGIDO
# ============================================================================
...
if generate_synthetic == 'y':
    ...
    if n_datasets > len(N0_samples):
        print(f"\n⚠ Warning: Requested {n_datasets} datasets but only {len(N0_samples)} posterior samples available.")
    ...
    print(f"\n✓ Selected {n_datasets} random posterior samples")
    ...
    for dataset_idx, sample_idx in enumerate(selected_sample_indices):
        ...
        # Generate cycles for each stress level using the Castillo-Canteli Weibull model
        cycles_synthetic = np.zeros(n_obs_per_dataset)

        for i in range(n_obs_per_dataset):
            stress = stress_synthetic[i]

            # Generate random Weibull variable V
            # Inverse CDF method for Weibull distribution
            u = np.random.uniform(0, 1)
            V = lambda_synth + delta_synth * ((-np.log(1 - u)) ** (1/beta_synth))

            # Calculate N from V = log(N/N0) * (stress/Delta0)
            # => N = N0 * exp(V * Delta0 / stress)
            if V > 0 and stress > 0:
                N = N0_synth * np.exp(V * Delta0_synth / stress)
            else:
                N = np.nan

            cycles_synthetic[i] = N

        # Create DataFrame for this dataset - CORRECCIÓN: Crear primero el DataFrame
        synthetic_df = pd.DataFrame({
            'N': cycles_synthetic,
            'Deltasigma': stress_synthetic
        })

        # Remove any NaN values ANTES de guardar
        synthetic_df = synthetic_df.dropna()

        # Verificar que tenemos datos después de eliminar NaN
        if len(synthetic_df) == 0:
            print(f"  ⚠ Dataset {dataset_idx + 1}: No valid data generated, skipping...")
            continue
        ...
        with open(r_filename, 'w') as f:
            ...
            f.write(f"# Parameters: N0={N0_synth:.6f}, Delta0={Delta0_synth:.6f}, lambda={lambda_synth:.3f}, delta={delta_synth:.3f}, beta={beta_synth:.3f}\n")
            f.write(f"list(M={len(synthetic_df)},\n")
            f.write(f"ns=50,\n")
            f.write(f"np=5,\n")
    ...
    # Verificar que tenemos al menos un dataset válido
    if len(all_synthetic_data) == 0:
        print(f"\n✗ ERROR: No valid synthetic datasets were generated!")
        print(f"  Possible issues:")
        print(f"  1. Parameter values may be invalid")
        print(f"  2. Stress/Delta0 ratio may cause numerical issues")
        print(f"  3. Weibull samples may be negative")
        generate_synthetic = 'n'  # Marcar como no generado
    else:
        print(f"\n✓ Generated {len(all_synthetic_data)} synthetic datasets!")
        print(f"  Files saved in: {synthetic_dir}/")

if generate_synthetic == 'y' and len(all_synthetic_data) > 0:
    # Continuar con el resto del código solo si tenemos datos
    # Create summary file with parameters used
    params_summary = pd.DataFrame({
        'dataset': [f'synthetic_{i+1:04d}' for i in range(len(all_synthetic_data))],
    ...
    print(f"✓ Parameters summary saved to: {params_summary_file}")
    ...
    print(f"\nGenerated datasets: {len(all_synthetic_data)}")
    print(f"Observations per dataset: {n_obs_per_dataset}")
    print(f"Valid observations generated: {len(consolidated_df)}")

    print(f"\nParameter ranges across datasets:")
    print(f"  N₀:     [{params_summary['N0'].min():.6f}, {params_summary['N0'].max():.6f}]")
    print(f"  Δσ₀:    [{params_summary['Delta0'].min():.6f}, {params_summary['Delta0'].max():.6f}]")
    print(f"  λ:      [{params_summary['lambda'].min():.3f}, {params_summary['lambda'].max():.3f}]")
    print(f"  δ:      [{params_summary['delta'].min():.3f}, {params_summary['delta'].max():.3f}]")
    print(f"  β:      [{params_summary['beta'].min():.3f}, {params_summary['beta'].max():.3f}]")

    # Visualize first few datasets
    print("\nVisualizing first 3 synthetic datasets vs observed data...")

    n_plots = min(3, len(all_synthetic_data))
    ...
    if n_plots > 0:
        ...
        # Definir límites comunes basados en los datos observados
        ...
        ax.set_ylabel('Stress (Δσ)', fontsize=10, fontweight='bold')
        ax.set_xscale('log')
        ...
        for i in range(n_plots):
            ...
            ax.set_ylabel('Stress (Δσ)', fontsize=10, fontweight='bold')
            ax.set_xscale('log')
            # Usar los mismos límites en todos los gráficos
            ax.set_xlim([common_x_min, common_x_max])
            ax.set_ylim([common_y_min, common_y_max])
        ...
        if IN_COLAB:
            ...
            print(f"\n✓ Files ready in: {synthetic_dir}/")
            print(f"✓ ZIP archive: {zip_filename}")

        print("\n" + "="*70)
        print("FILES IN ZIP ARCHIVE:")
        print("="*70)
        print(f"  • synthetic_XXXX.csv ({len(all_synthetic_data)} files) - Individual datasets in CSV format")
        print(f"  • synthetic_XXXX.txt ({len(all_synthetic_data)} files) - Individual datasets in R/OpenBUGS format")
        print(f"  • all_synthetic_data.csv - All datasets combined")
        print(f"  • parameters_summary.csv - Parameters used for each dataset")
        print(f"  • synthetic_vs_observed_comparison.png - Visual comparison")

        print("\n✓ Synthetic data generation complete!")
    else:
        print("\n⚠ No synthetic datasets to visualize")
...
# ============================================================================
# CREATE ZIP WITH ALL RESULTS (INCLUDING FULL PYTHON SCRIPT) - VERSIÓN CORREGIDA
# ============================================================================
...
create_zip = input("\n¿Crear archivo ZIP con todos los resultados? (y/n): ").strip().lower()
...
if create_zip == 'y':
    ...
    print(f"\n💾 Creando archivo .py con información completa...")
...
inference using the Castillo-Canteli 3-parameter Weibull model for fatigue data analysis.

RESULTS
-------
MLE PARAMETERS:
  N₀ (reference cycles) = {mle_params[0]:.6f}
  Δσ₀ (reference stress) = {mle_params[1]:.6f}
  λ (location parameter) = {mle_params[2]:.6f}
  δ (scale parameter)    = {mle_params[3]:.6f}
  β (shape parameter)    = {mle_params[4]:.6f}
  Log-likelihood         = {-result_global.fun:.2f}

BAYESIAN POSTERIOR MEDIANS:
  N₀ = {posterior['N0'].median().values:.6f}
  Δσ₀ = {posterior['Delta0'].median().values:.6f}
  λ = {posterior['lambda_param'].median().values:.3f}
  δ = {posterior['delta'].median().values:.3f}
  β = {posterior['beta'].median().values:.3f}
...
CONVERGENCE DIAGNOSTICS:
  R-hat all < 1.01: {'✓' if all_rhat_good else '✗'}
  ESS all > 1000: {'✓' if all_ess_good else '✗'}
...
PRIOR DISTRIBUTIONS (CV):
  CV(N₀) = {config['cv_N0']:.2f}
  CV(Δσ₀) = {config['cv_Delta0']:.2f}
  CV(λ) = {config['cv_lambda']:.2f}
  CV(δ) = {config['cv_delta']:.2f}
  CV(β) = {config['cv_beta']:.2f}
...
NOTES:
------
• This analysis uses the Castillo-Canteli 3-parameter Weibull distribution
• Dimensionless formulation V = log(N/N₀) * (Δσ/Δσ₀)
• Informative priors centered on MLE estimates
...
print("CONVERGENCE DIAGNOSTICS")
print("="*70)
print(f"R-hat all < 1.01: {'✓ PASS' if all_rhat_good else '✗ FAIL'}")
print(f"ESS all > 1000: {'✓ PASS' if all_ess_good else '✗ FAIL'}")
print(f"Divergences: {n_divergences}")
print(f"Divergence rate: {n_divergences/total_samples*100:.2f}%")
...
if __name__ == "__main__":
    ...
    print("• load_summary() - Loads and displays statistical summary")
'''
    ...
    print(f"✓ Archivo informativo creado: {py_script_filename}")

    # ========================================================================
    # 2. GUARDAR EL SCRIPT PYTHON COMPLETO
    # ========================================================================
    print(f"\n💾 Guardando script Python COMPLETO...")

    # Intentar obtener el código fuente actual
    try:
        # Para Google Colab
        if IN_COLAB:
            try:
                # Método 1: Usar IPython para obtener el historial
                from IPython import get_ipython
                ipython = get_ipython()
                if ipython:
                    # Obtener todas las celdas ejecutadas
                    current_code = ""
                    # Intentar obtener el código de varias formas
                    try:
                        # Método para notebooks
                        ...
                        # Método alternativo: capturar el historial de comandos
                        history = ipython.history_manager.get_range()
                        for h in history:
                            current_code += h[2] + "\n"
            except Exception as e:
                print(f"⚠ No se pudo obtener el código completo de Colab: {e}")
                current_code = ""
        else:
            # Para ejecución local desde archivo .py
            try:
                import inspect
                import __main__
                # Intentar obtener el código fuente del módulo principal
    ...
    # Si no pudimos obtener el código, crear uno representativo
    if not current_code or len(current_code) < 1000:
        print("⚠ No se pudo obtener el código completo, creando versión representativa...")
        current_code = '''#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
FATIGUE ANALYSIS - WEIBULL MODEL - SCRIPT COMPLETO
==================================================
Script principal para análisis de fatiga usando modelo Weibull de Castillo-Canteli.

NOTA: Este es un archivo representativo. El código original se ejecutó
en un entorno interactivo (Colab/Jupyter) y no se pudo capturar completo.

Para reproducir el análisis completo, consulte:
1. Los archivos de resultados generados
2. Las imágenes con visualizaciones
3. Los datos sintéticos (si se generaron)
4. El archivo analysis_info.py para parámetros detallados
"""

print("Script representativo del análisis de fatiga Weibull Castillo-Canteli")
print("El análisis completo incluye:")
print("1. Estimación de Máxima Verosimilitud (MLE) con modelo 3-parámetros Weibull")
print("2. Inferencia Bayesiana con PyMC")
print("3. Cálculo de percentiles de percentiles")
print("4. Generación de visualizaciones")
print("5. Opcional: Generación de datos sintéticos")
'''
...
DESCRIPCIÓN:
------------
Análisis completo de fatiga usando modelo Weibull 3-parámetros con formulación
adimensional de Castillo-Canteli.

ESTE ES EL SCRIPT COMPLETO QUE SE EJECUTÓ PARA GENERAR LOS RESULTADOS.
Incluye todos los pasos del análisis:
...
2. ESTIMACIÓN DE MÁXIMA VEROSIMILITUD (MLE)
   - Optimización global con differential_evolution
   - Cálculo de parámetros del modelo Weibull 3-parámetros

3. INFERENCIA BAYESIANA
   - Modelo con PyMC usando priors informativos
   - MCMC sampling con múltiples cadenas
...
6. OPCIONAL: DATOS SINTÉTICOS
   - Generación de datasets sintéticos
   - Basados en muestras posteriores

PARÁMETROS CLAVE:
-----------------
• N₀: Ciclos de referencia = {mle_params[0]:.6f}
• Δσ₀: Tensión de referencia = {mle_params[1]:.6f}
• λ: Parámetro de localización = {mle_params[2]:.3f}
• δ: Parámetro de escala = {mle_params[3]:.3f}
• β: Parámetro de forma = {mle_params[4]:.3f}
...
USO:
----
Para ejecutar el análisis completo, asegúrese de tener instaladas
...
print(f"• Observaciones procesadas: {len(cycles_data)}")
print(f"• Parámetros MLE estimados: N₀={mle_params[0]:.6f}, Δσ₀={mle_params[1]:.6f}, λ={mle_params[2]:.3f}")
print(f"• Muestras MCMC generadas: {config['n_draws'] * config['n_chains']}")
print(f"• Percentiles calculados: {len(config['percentiles_base'])} base × {len(config['percentiles_sub'])} sub")
{'• Datos sintéticos generados: ' + str(n_datasets) + ' datasets' if 'n_datasets' in locals() else '• No se generaron datos sintéticos'}

print("\\n¡Gracias por usar el sistema de análisis de fatiga Weibull Castillo-Canteli!")
'''
        f.write(footer)

    print(f"✓ Script COMPLETO guardado: {full_script_filename}")
    ...
    files_to_zip = []
    print(f"\n📦 Preparando {len(main_files)} archivos principales...")
    for original_file, short_name in main_files:
        if os.path.exists(original_file):
            files_to_zip.append((original_file, short_name))
            print(f"  ✓ {short_name}")
        else:
            print(f"  ✗ {short_name} (no encontrado)")

    # Añadir directorio de datos sintéticos si existe
    if synthetic_dir and os.path.exists(synthetic_dir):
        print(f"\n📁 Añadiendo datos sintéticos...")
        synth_file_count = 0
        for root, dirs, files in os.walk(synthetic_dir):
            for file in files:
                file_path = os.path.join(root, file)
                ...
                synth_file_count += 1
        print(f"  ✓ {synth_file_count} archivos sintéticos añadidos")

    # ========================================================================
    # 4. CREAR EL ARCHIVO ZIP
    # ========================================================================
    if files_to_zip:
        print(f"\n📦 Creando ZIP '{zip_filename}' con {len(files_to_zip)} archivos...")
        ...
        print(f"   📄 Total archivos: {len(files_to_zip)}")

        zip_files_created.append(zip_filename)

        # ====================================================================
        # 5. MOSTRAR CONTENIDO DEL ZIP (VERSIÓN CORREGIDA)
        # ====================================================================
        print("\n" + "="*60)
        ...
        with zipfile.ZipFile(zip_filename, 'r') as zipf:
            ...
            # Primero definir las categorías principales
        ...
        if IN_COLAB:
            ...
            try:
                files.download(zip_filename)
                print(f"✅ ZIP descargado: {zip_filename}")
                print(f"   📦 Tamaño: {total_size/(1024*1024):.2f} MB")
                print(f"   📄 Archivos incluidos: {len(file_list)}")
                print(f"\n⚠ IMPORTANTE: El archivo 'fatigue_analysis_FULL.py' contiene")
                print("              el código Python COMPLETO del análisis")
            except Exception as e:
                print(f"⚠ No se pudo descargar automáticamente: {e}")
                print(f"📍 Archivo disponible en: {os.path.abspath(zip_filename)}")
        else:
            print(f"\n📍 Archivo ZIP disponible en: {os.path.abspath(zip_filename)}")
            print(f"   📦 Tamaño: {total_size/(1024*1024):.2f} MB")
            print(f"   📄 Archivos incluidos: {len(file_list)}")
            print(f"\n⚠ IMPORTANTE: El archivo 'fatigue_analysis_FULL.py' contiene")
            print("              el código Python COMPLETO del análisis")
    ...
    print("\n✓ Saltando creación de ZIP")

# ============================================================================
# LIMPIEZA TOTAL AUTOMÁTICA
# ============================================================================

print("\n" + "="*70)
print("LIMPIANDO TODO AUTOMÁTICAMENTE")
print("="*70)
...
no_eliminar = [
    # El script principal si estamos ejecutando desde él
    __file__ if '__file__' in globals() else '',

    # Archivos Python del sistema
    '*.py'  # Pero cuidado, solo eliminaremos los específicos arriba
]
...
for patron in patrones_eliminar:
    for archivo in glob.glob(patron):
        # Verificar que no esté en la lista de no eliminar
        eliminar = True
...
if archivos_eliminados:
    print(f"\n🗑️  Eliminados {len(archivos_eliminados)} archivos:")
    ...
    print(f"\n💾 Espacio liberado: {espacio/(1024*1024):.1f} MB")
else:
...
# MODELOWEIBULL
# ============================================================================
#   FATIGUE ANALYSIS - GEV MINIMA MODEL (WEIBULL) - ARITHMETIC STRESS SCALE
#   Castillo-Canteli Dimensionless Formulation
#   WITH GOOGLE COLAB FILE UPLOAD SUPPORT
# ============================================================================
# Physical justification:
# - Lower bounded problem (N > 0) → GEV minima distribution (Weibull type)
# - GEV minima: F(x) = 1 - exp(-[1 + ξ((x-μ)/σ)]^(-1/ξ)) for ξ ≠ 0
# - Weibull case: ξ > 0 (lower bounded distribution)
# ============================================================================
...
print("="*70)
print("FATIGUE ANALYSIS - GEV MINIMA MODEL (WEIBULL)")
print("Maximum Likelihood Estimation + Bayesian Inference")
print("Arithmetic Stress Scale (no logarithms for Deltasigma)")
print("="*70)
print(f"PyMC version: {pm.__version__}")
print(f"ArviZ version: {az.__version__}")
...
if len(existing_files) > 0:
    ...
    if confirm == 'y':
        ...
        print("\n✓ Keeping existing files (will be overwritten)")
else:
...
if use_example:
    ...
    config = {
        ...
        'n_stress_points': 50,
        'n_param_samples': 1000,
        'percentiles_base': [0.0, 0.15, 0.50, 0.85, 0.99],
        'percentiles_sub': [0.0, 0.15, 0.50, 0.85, 0.99],
        'cv_N0': 0.25,
        'cv_Delta0': 0.15,
        'cv_beta': 0.30,
        'cv_lambda': 0.25,
    ...
    if not use_example:
        ...
        try:
            ...
            if filename.endswith(('.txt', '.dat')):
                ...
                # Coefficient of variation parameters
                cv_params = ['cv_N0', 'cv_Delta0', 'cv_beta', 'cv_lambda', 'cv_delta', 'cv_xi']
                for cv_param in cv_params:
                    cv_match = re.search(rf'{cv_param}\s*=\s*([\d.]+)', content)
                    if cv_match:
                        config[cv_param] = float(cv_match.group(1))
...
print(f"Data source: {'Holmen example (built-in)' if use_example else 'User-uploaded file'}")
print(f"Observations: {len(cycles_data)}")
print(f"Stress range: [{stress_data.min():.3f}, {stress_data.max():.3f}] (dimensionless, ARITHMETIC)")
print(f"Cycles range: [{N_min:.4f}, {N_max:.2f}]")
print(f"\nFirst 5 N values: {cycles_data[:5]}")
print(f"First 5 Deltasigma values: {stress_data[:5]}")
...
print(f"  CV(λ) = {config['cv_lambda']:.2f}")
print(f"  CV(δ) = {config['cv_delta']:.2f}")
print(f"  CV(ξ) = {config['cv_xi']:.2f}  (GEV shape parameter)")
...
ax.set_ylabel('Stress (Deltasigma, dimensionless)', fontsize=12, fontweight='bold')
ax.set_xscale('log')
ax.set_title(f'S-N Data ({dataset_name}) - GEV Minima Model', fontsize=13, fontweight='bold')
if len(unique_stresses) <= 10:
    ax.legend(fontsize=9, loc='upper right')
ax.grid(True, alpha=0.3, which='both')

ax = axes[1]
ax.scatter(np.log(cycles_data), stress_data, alpha=0.6, s=40, color='darkblue')
ax.set_xlabel('ln(Cycles)', fontsize=12, fontweight='bold')
ax.set_ylabel('Stress (arithmetic)', fontsize=12, fontweight='bold')
ax.set_title('Cycles Log, Stress Arithmetic', fontsize=13, fontweight='bold')
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('data_exploration_gev.png', dpi=150, bbox_inches='tight')
plt.show()

print("✓ Data visualization completed!")

# ============================================================================
# PHASE 1: MAXIMUM LIKELIHOOD ESTIMATION (GEV MINIMA)
# ============================================================================

print("\n" + "="*70)
print("PHASE 1: MAXIMUM LIKELIHOOD ESTIMATION (GEV MINIMA)")
print("="*70)

def gev_minima_log_likelihood(params, stress, cycles):
    """
    GEV minima log-likelihood for fatigue data.
    Castillo-Canteli dimensionless formulation.
    Using ARITHMETIC stress scale (no logarithms for Deltasigma).

    Model for minima (lower bounded data):
    log(N) ~ GEV minima with location-scale depending on stress

    GEV minima distribution (Weibull type: ξ > 0):
    F(x) = 1 - exp(-[1 + ξ((x-μ)/σ)]^(-1/ξ)) for 1 + ξ((x-μ)/σ) > 0

    Parameters:
    -----------
    params : array [N0, Delta0, beta, lambda_param, delta, xi]
        N0: reference number of cycles
        Delta0: reference stress (endurance limit) - ARITHMETIC SCALE
        beta: scale parameter
        lambda_param: location parameter
        delta: shape parameter 1
        xi: GEV shape parameter (ξ > 0 for Weibull type)
    """
    N0, Delta0, beta, lambda_param, delta, xi = params

    # Validations
    if N0 <= 0 or Delta0 <= 0 or beta <= 0 or delta <= 0:
        return -np.inf

    # ξ should be positive for Weibull type (lower bounded)
    if xi <= -0.5 or xi >= 5.0:  # Reasonable bounds for ξ in fatigue
        return -np.inf

    # Dimensionless transformation
    # IMPORTANT: Using ARITHMETIC stress difference (not logarithm)
    log_N_dimensionless = np.log(cycles) - np.log(N0)
    r = stress - Delta0  # ARITHMETIC difference, not logarithmic

    # Check for valid r values
    if np.any(np.abs(r) < 1e-10):
        return -np.inf

    # GEV parameters for minima
    # Location parameter
    mu_Y = (-lambda_param - delta) / r
    # Scale parameter
    sigma_Y = delta / (beta * np.abs(r))

    # Standardized variable for GEV minima
    z = (log_N_dimensionless - mu_Y) / sigma_Y

    # Check domain condition: 1 + ξ*z > 0
    if np.any(1 + xi * z <= 0):
        return -np.inf

    # GEV minima log-likelihood
    # PDF: f(x) = (1/σ) * [1 + ξ((x-μ)/σ)]^(-1/ξ - 1) * exp(-[1 + ξ((x-μ)/σ)]^(-1/ξ))
    # For minima: Using transformation z = (x-μ)/σ

    if abs(xi) < 1e-10:  # Gumbel case (ξ → 0)
        # Gumbel minima: f(x) = (1/σ) * exp(z - exp(z))
        log_lik = -np.log(sigma_Y) + z - np.exp(z)
    else:
        # GEV minima with ξ ≠ 0
        t = 1 + xi * z
        log_lik = -np.log(sigma_Y) - (1/xi + 1) * np.log(t) - np.power(t, -1/xi)
...
def negative_log_likelihood(params, stress, cycles):
    """Negative log-likelihood for minimization."""
    return -gev_minima_log_likelihood(params, stress, cycles)
...
lambda_init = -8.0
delta_init = 2.0
xi_init = 0.1  # Small positive ξ for Weibull type

initial_params = np.array([N0_init, Delta0_init, beta_init, lambda_init, delta_init, xi_init])

print("\nInitial guess:")
print(f"  N₀ = {N0_init:.4f}")
print(f"  Δσ₀ = {Delta0_init:.4f}  (arithmetic scale)")
print(f"  β = {beta_init:.4f}")
print(f"  λ = {lambda_init:.4f}")
print(f"  δ = {delta_init:.4f}")
print(f"  ξ = {xi_init:.4f}  (GEV shape, >0 for Weibull)")

# Parameter bounds for optimization
bounds = [
    (0.001, N_min * 0.9),           # N0
    (stress_data.min() * 0.4, stress_data.min() * 0.99),  # Delta0 (arithmetic)
    (0.5, 15.0),                    # beta
    (-12.0, -4.0),                  # lambda
    (0.5, 5.0),                     # delta
    (0.001, 2.0)                    # xi (positive for Weibull type)
]
...
print("\n" + "="*70)
print("MLE ESTIMATES (GEV MINIMA MODEL - ARITHMETIC STRESS)")
print("="*70)
print(f"  N₀ (reference cycles)   = {mle_params[0]:.6f}")
print(f"  Δσ₀ (reference stress)  = {mle_params[1]:.6f}  (arithmetic scale)")
print(f"  β (scale parameter)     = {mle_params[2]:.6f}")
print(f"  λ (location param)      = {mle_params[3]:.6f}")
print(f"  δ (shape param 1)       = {mle_params[4]:.6f}")
print(f"  ξ (GEV shape param)     = {mle_params[5]:.6f}  (>0: Weibull type)")
print(f"\n  Log-likelihood = {-result_global.fun:.2f}")

# Save MLE results
with open('mle_estimates.txt', 'w') as f:
    f.write(f"MLE ESTIMATES - GEV MINIMA MODEL - {dataset_name}\n")
    f.write("="*50 + "\n")
    f.write(f"N0 = {mle_params[0]:.8f}\n")
    f.write(f"Delta0 = {mle_params[1]:.8f}\n")
...
# Plot MLE curves for different percentiles
stress_range_plot = np.linspace(stress_data.min() * 0.97, stress_data.max() * 1.03, 100)
percentiles_mle = [0.0, 0.10, 0.50, 0.90, 0.99]
colors_mle = ['#8B0000', '#FF8C00', '#228B22', '#4169E1', '#8B008B']

N0_mle, Delta0_mle, beta_mle, lambda_mle, delta_mle, xi_mle = mle_params

def compute_gev_quantile(stress, N0, Delta0, beta, lambda_p, delta, xi, prob):
    """Compute N_p for given stress and failure probability (GEV minima)."""
    ...
    try:
        r = stress - Delta0  # ARITHMETIC difference
        if abs(r) < 1e-10:
            return np.nan
        ...
        if abs(xi) < 1e-10:  # Gumbel case
            ...
            z_p = (np.power(-np.log(1 - prob), -xi) - 1) / xi
...
for perc, color in zip(percentiles_mle, colors_mle):
    N_perc = []
    for stress in stress_range_plot:
        N_p = compute_gev_quantile(stress, N0_mle, Delta0_mle, beta_mle,
                                   lambda_mle, delta_mle, xi_mle, perc)
        N_perc.append(N_p)
...
ax.set_ylabel('Stress (Deltasigma, dimensionless)', fontsize=13, fontweight='bold')
ax.set_xscale('log')
ax.set_title(f'MLE Fit - GEV Minima Model, ξ={xi_mle:.3f} ({dataset_name})',
            fontsize=14, fontweight='bold')
ax.legend(fontsize=10, loc='upper right')
ax.grid(True, alpha=0.3, which='both')
ax.set_xlim([N_min * 0.1, N_max * 10])
...
lambda_std = abs(lambda_mle_val) * config['cv_lambda']
delta_std = delta_mle_val * config['cv_delta']
xi_std = abs(xi_mle_val) * config['cv_xi']
...
print(f"  λ      ~ Normal({lambda_mle_val:.4f}, {lambda_std:.4f})  [CV={config['cv_lambda']:.2f}]")
print(f"  δ      ~ Normal({delta_mle_val:.4f}, {delta_std:.4f})  [CV={config['cv_delta']:.2f}]")
print(f"  ξ      ~ Normal({xi_mle_val:.4f}, {xi_std:.4f})  [CV={config['cv_xi']:.2f}]  (GEV shape)")

print("\nThese priors will help achieve faster convergence in Bayesian inference.")

# ============================================================================
# PHASE 3: BAYESIAN INFERENCE WITH GEV MINIMA MODEL
# ============================================================================

print("\n" + "="*70)
print("PHASE 3: BAYESIAN INFERENCE (GEV MINIMA)")
print("="*70)
...
with pm.Model() as fatigue_model:
    ...
    delta = pm.TruncatedNormal('delta', mu=delta_mle_val, sigma=delta_std,
                              lower=0.1, upper=10.0)
    xi = pm.TruncatedNormal('xi', mu=xi_mle_val, sigma=xi_std,
                           lower=0.001, upper=2.0)  # ξ > 0 for Weibull type

    # Transform to dimensionless log-space for cycles
    # Using ARITHMETIC stress difference
    log_N_dimensionless = pt.log(cycles_data) - pt.log(N0)
    r = stress_data - Delta0  # ARITHMETIC difference
    ...
    # Domain condition for GEV: 1 + ξ*z > 0
    domain_condition = pt.all(1 + xi * z > 0)

    # Log-likelihood for GEV minima
    # Two cases: ξ = 0 (Gumbel) and ξ ≠ 0 (GEV)
    xi_abs = pt.abs(xi)

    # Use piecewise function for numerical stability
    log_lik_gumbel = -pt.log(sigma_Y + 1e-8) + z - pt.exp(z)

    # GEV with ξ ≠ 0
    t = 1 + xi * z
    log_lik_gev = -pt.log(sigma_Y + 1e-8) - (1/xi + 1) * pt.log(t) - pt.pow(t, -1/xi)

    # Choose based on ξ value (use smooth transition for numerical stability)
    # When |ξ| < 0.001, use Gumbel approximation
    use_gumbel = pt.lt(xi_abs, 0.001)
    log_lik = pt.switch(use_gumbel, log_lik_gumbel, log_lik_gev)

    # Add domain condition as potential
    pm.Potential('domain', pt.switch(domain_condition, 0, -np.inf))
...
var_names = ['N0', 'Delta0', 'beta', 'lambda_param', 'delta', 'xi']
var_labels = ['N₀ (reference cycles)', 'Δσ₀ (reference stress)', 'β', 'λ', 'δ', 'ξ (GEV shape)']

for ax, var, label in zip(axes[:6], var_names, var_labels):
    samples = prior_predictive.prior[var].values.flatten()
    ...
    if var == 'N0':
        ...
        ax.axvline(xi_mle_val, color='green', linestyle=':', linewidth=2, label='MLE')
...
print(f"\n--- R-hat Diagnostic (should be < 1.01) ---")
all_rhat_good = True
for var in ['N0', 'Delta0', 'beta', 'lambda_param', 'delta', 'xi']:
    rhat_val = rhat_values[var].values
    status = "✓" if rhat_val < 1.01 else "✗"
    if rhat_val >= 1.01:
        all_rhat_good = False
...
def compute_gev_percentile(stress, N0, Delta0, beta, lambda_p, delta, xi, prob):
    """Compute N_p for given stress and failure probability (GEV minima)."""
    if stress <= 0 or N0 <= 0 or Delta0 <= 0 or beta <= 0 or delta <= 0:
        return np.nan
    if prob <= 0 or prob >= 1:
...
for perc_base_idx, perc_base in enumerate(percentiles_base):
    ...
    for i, stress in enumerate(stress_range):
        ...
        for j, idx in enumerate(sample_indices):
            ...
            N_p = compute_gev_percentile(stress, N0_s, Delta0_s, beta_s, lambda_s, delta_s, xi_s, perc_base)
...
ax.set_xscale('log')
ax.set_title(f'Percentiles of Percentiles with Uncertainty Bands ({dataset_name})\n' +
            'Bayesian GEV Minima Model (Weibull type) - Arithmetic Stress Scale',
            fontsize=15, fontweight='bold', pad=20)
...
ax.set_xscale('log')
ax.set_title(f'All {total_curves} Percentile Curves ({len(percentiles_base)} Base × {len(percentiles_sub)} Sub) - {dataset_name}\n' +
            'Complete Uncertainty Quantification - GEV Minima Model',
            fontsize=15, fontweight='bold', pad=20)
...
print("\n1. MLE ESTIMATES (GEV MINIMA - ARITHMETIC STRESS):")
print(f"   N₀ = {mle_params[0]:.6f}")
print(f"   Δσ₀ = {mle_params[1]:.6f}  (arithmetic scale)")
print(f"   β = {mle_params[2]:.3f}")
print(f"   λ = {mle_params[3]:.3f}")
print(f"   δ = {mle_params[4]:.3f}")
print(f"   ξ = {mle_params[5]:.3f}  (>0: Weibull type)")
...
print(f"   λ = {posterior['lambda_param'].median().values:.3f}")
print(f"   δ = {posterior['delta'].median().values:.3f}")
print(f"   ξ = {posterior['xi'].median().values:.3f}")
...
print("\n5. FILES CREATED:")
print("   • data_exploration_gev.png")
print("   • mle_results.png")
print("   • mle_estimates.txt")
print("   • prior_distributions.png")
print("   • trace_plots.png")
...
# Save results
trace.to_netcdf('fatigue_posterior_gev.nc')
summary_table.to_csv('fatigue_summary_gev.csv')
...
print("\n✓ Results saved:")
print("  • fatigue_posterior_gev.nc")
print("  • fatigue_summary_gev.csv")
print("  • percentiles_of_percentiles.npz")
...
if generate_synthetic == 'y':
    ...
    for dataset_idx, sample_idx in enumerate(selected_sample_indices):
        ...
        lambda_synth = lambda_samples[sample_idx]
        delta_synth = delta_samples[sample_idx]
        xi_synth = xi_samples[sample_idx]
        ...
        # Generate cycles for each stress level using the GEV minima model
        cycles_synthetic = np.zeros(n_obs_per_dataset)

        for i in range(n_obs_per_dataset):
            stress = stress_synthetic[i]

            # GEV parameters for this stress level
            r = stress - Delta0_synth  # ARITHMETIC difference
            mu_Y = (-lambda_synth - delta_synth) / r
            sigma_Y = delta_synth / (beta_synth * abs(r))

            # Generate random GEV minima variable
            u = np.random.uniform(0, 1)

            # Inverse CDF of GEV minima
            if abs(xi_synth) < 1e-10:  # Gumbel case
                z = np.log(-np.log(1 - u))
            else:  # GEV case
                z = (np.power(-np.log(1 - u), -xi_synth) - 1) / xi_synth
        ...
        synthetic_df.attrs['lambda'] = lambda_synth
        synthetic_df.attrs['delta'] = delta_synth
        synthetic_df.attrs['xi'] = xi_synth
        synthetic_df.attrs['sample_idx'] = sample_idx
        ...
        with open(r_filename, 'w') as f:
            f.write(f"# Synthetic dataset {dataset_idx+1}/{n_datasets}\n")
            f.write(f"# Generated from posterior sample {sample_idx}\n")
            f.write(f"# Parameters: N0={N0_synth:.6f}, Delta0={Delta0_synth:.6f}, beta={beta_synth:.3f}, lambda={lambda_synth:.3f}, delta={delta_synth:.3f}, xi={xi_synth:.3f}\n")
    ...
    print(f"\n✓ Generated {n_datasets} synthetic datasets!")
    print(f"  Files saved in: {synthetic_dir}/")
    ...
    params_summary = pd.DataFrame({
        ...
        'lambda': [df.attrs['lambda'] for df in all_synthetic_data],
        'delta': [df.attrs['delta'] for df in all_synthetic_data],
        'xi': [df.attrs['xi'] for df in all_synthetic_data]
    })
    ...
    print(f"  λ:      [{params_summary['lambda'].min():.3f}, {params_summary['lambda'].max():.3f}]")
    print(f"  δ:      [{params_summary['delta'].min():.3f}, {params_summary['delta'].max():.3f}]")
    print(f"  ξ:      [{params_summary['xi'].min():.3f}, {params_summary['xi'].max():.3f}]")
    ...
    print("FILES IN ZIP ARCHIVE:")
    print("="*70)
    print(f"  • synthetic_XXXX.csv ({n_datasets} files) - Individual datasets in CSV format")
    print(f"  • synthetic_XXXX.txt ({n_datasets} files) - Individual datasets in R/OpenBUGS format")
    print(f"  • all_synthetic_data.csv - All datasets combined")
    print(f"  • parameters_summary.csv - Parameters used for each dataset")
    print(f"  • synthetic_vs_observed_comparison.png - Visual comparison")
...
if IN_COLAB:
    ...
    files_to_download = [
        ...
        'fatigue_posterior_gev.nc',
        'fatigue_summary_gev.csv',
...
# Physical justification:
# - Lower bounded problem (N > 0) → Weibull distribution for minima
# - Gumbel is limiting case (β → ∞) naturally captured by Weibull
# ============================================================================
...
print(f"  N₀ (reference cycles) = {mle_params[0]:.6f}")
print(f"  Δσ₀ (reference stress) = {mle_params[1]:.6f}")
print(f"  β (shape parameter)    = {mle_params[2]:.6f}")
print(f"  λ (location param)     = {mle_params[3]:.6f}")
print(f"  δ (scale param)        = {mle_params[4]:.6f}")
print(f"\n  Log-likelihood = {-result_global.fun:.2f}")
...
var_labels = ['N₀ (reference cycles)', 'Δσ₀ (reference stress)', 'β', 'λ', 'δ']
...
zip_files_created = []
py_script_filename = None
synthetic_zip_created = None  # Para rastrear si se creó un ZIP de datos sintéticos
...
if create_zip == 'y':
    ...
    # Crear archivo .py COMPLETO con información
    py_script_filename = f"analysis_info_{dataset_name[:10]}_{timestamp}.py"
    print(f"\n💾 Creando archivo .py con información completa...")
...
MLE PARAMETERS:
  N₀ (reference cycles) = {mle_params[0]:.6f}
  Δσ₀ (reference stress) = {mle_params[1]:.6f}
  β (shape parameter)    = {mle_params[2]:.6f}
  λ (location param)     = {mle_params[3]:.6f}
  δ (scale param)        = {mle_params[4]:.6f}
  Log-likelihood         = {-result_global.fun:.2f}
...
NOTES:
------
• This analysis uses the Weibull distribution for minima (fatigue data)
• Castillo-Canteli dimensionless formulation
• Informative priors centered on MLE estimates
• Bayesian uncertainty quantification
...
'''
    ...
    print(f"✓ Python file created (complete): {py_script_filename}")
    ...
    # Añadir directorio de datos sintéticos SI EXISTE y NO se creó ZIP separado
    # (Quitamos la creación del ZIP separado de datos sintéticos)
    if synthetic_dir and os.path.exists(synthetic_dir):
        # Incluir todo el directorio sintético en el ZIP general
        for root, dirs, files in os.walk(synthetic_dir):
            for file in files:
                file_path = os.path.join(root, file)
    ...
    if files_to_zip:
        print(f"\n📦 Files to compress ({len(files_to_zip)}):")
        ...
        if synth_files > 0:
            print(f"  🔢 Synthetic data files: {synth_files}")
        ...
        with zipfile.ZipFile(zip_filename, 'r') as zipf:
            ...
            # Primero definir todas las categorías excepto 'Other'
            categories = {
            ...
            # Ahora calcular 'Other' usando las categorías ya definidas
            all_categorized = []
        ...
        if IN_COLAB:
            ...
            try:
                files.download(zip_filename)
                print(f"✅ ZIP file downloaded: {zip_filename}")
            ...
            print(f"\n📍 ZIP file available at: {os.path.abspath(zip_filename)}")
            print(f"   Size: {total_size/(1024*1024):.2f} MB")
            print(f"   Files: {len(file_list)}")
...
# ELIMINAR cualquier ZIP de datos sintéticos que se haya creado
# (porque ya están incluidos en el ZIP general)
if 'zip_filename' in locals() and os.path.exists(zip_filename):
    # Buscar y eliminar ZIPs de datos sintéticos separados
    synth_zip_pattern = f"synthetic_data_{dataset_name}*.zip"
    for synth_zip in glob.glob(synth_zip_pattern):
        try:
            os.remove(synth_zip)
            print(f"  ✓ Removed duplicate synthetic ZIP: {os.path.basename(synth_zip)}")
        except:
            pass
...
if zip_files_created:
    ...
    if keep_main_zip == 'y':
        print("✓ Main ZIP file will be kept.")
        ...
        print("✓ Main ZIP file will be deleted.")
...
if existing_files:
    ...
    for f in existing_files:
        if os.path.isdir(f):
            ...
            print(f"  📁 {f}/ ({file_count} files, {dir_size/1024:.1f} KB)")
        else:
        ...
        for filepath in existing_files:
            try:
                if os.path.isdir(filepath):
                    ...
                    print(f"  ✓ Deleted directory: {filepath}/ ({dir_size/1024:.1f} KB)")
                else:
                    file_size = os.path.getsize(filepath)
                    os.remove(filepath)
                    removed_count += 1
                    removed_size += file_size
                    print(f"  ✓ Deleted: {os.path.basename(filepath)} ({file_size/1024:.1f} KB)")
            except Exception as e:
                print(f"  ✗ Error deleting {filepath}: {e}")

        print(f"\n✅ Cleanup completed:")
        print(f"   • Files/directories deleted: {removed_count}")
        print(f"   • Space freed: {removed_size/(1024*1024):.2f} MB")

    else:
        print("\n✅ Operation cancelled. Files kept.")
...
FATIGUE ANALYSIS - WEIBULL MODEL - SCRIPT COMPLETO
==================================================
Script principal para análisis de fatiga usando modelo Weibull.
...
print("Script representativo del análisis de fatiga Weibull")
print("El análisis completo incluye:")
print("1. Estimación de Máxima Verosimilitud (MLE)")
print("2. Inferencia Bayesiana con PyMC")
print("3. Cálculo de percentiles de percentiles")
print("4. Generación de visualizaciones")
print("5. Opcional: Generación de datos sintéticos")
...
DESCRIPCIÓN:
------------
Análisis completo de fatiga usando modelo Weibull con formulación
adimensional de Castillo-Canteli.
...
2. ESTIMACIÓN DE MÁXIMA VEROSIMILITUD (MLE)
   - Optimización global con differential_evolution
   - Cálculo de parámetros del modelo Weibull
...
• N₀: Ciclos de referencia = {mle_params[0]:.6f}
• Δσ₀: Tensión de referencia = {mle_params[1]:.6f}
• β: Parámetro de forma = {mle_params[2]:.3f}
• λ: Parámetro de localización = {mle_params[3]:.3f}
• δ: Parámetro de escala = {mle_params[4]:.3f}
...
print("\\nRESUMEN DE RESULTADOS:")
print(f"• Observaciones procesadas: {len(cycles_data)}")
print(f"• Parámetros MLE estimados: N₀={mle_params[0]:.6f}, Δσ₀={mle_params[1]:.6f}")
print(f"• Muestras MCMC generadas: {config['n_draws'] * config['n_chains']}")
print(f"• Percentiles calculados: {len(config['percentiles_base'])} base × {len(config['percentiles_sub'])} sub")
{'• Datos sintéticos generados: ' + str(n_datasets) + ' datasets' if 'n_datasets' in locals() else '• No se generaron datos sintéticos'}

print("\\n¡Gracias por usar el sistema de análisis de fatiga Weibull!")
'''
        f.write(footer)
